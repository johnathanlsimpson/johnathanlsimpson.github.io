<article class="slide slide--rarezhut">
	<section class="work__container work__images middled">
		<div class="work__images-container">
			<div class="work__image" style="background-image:url(images/img-rarezhut.png); grid-column: 1 / span 20; grid-row: 4 / span 17;"></div>
		</div>
	</section>

	<section class="work__container work__details middled">
		<a class="work__title text--spaced a--filled a--alt-filled a--attention" href="https://rarezhut.net/" target="_blank">
			RarezHu<l>t</l>
		</a>

		<div class="work__summary">
			<p class="work__description text--secondary">
				<span class="work__description-bg">Developed custom PHP/JS live auction system for online record shop.</span>
				<span class="work__tech">PHP7</span>
				<span class="work__tech">SQL</span>
				<span class="work__tech">JS ES6</span>
				<span class="work__tech">HTML5</span>
				<span class="work__tech">CSS3</span>
			</p>
		</div>

		<a class="a--circled a--filled text--primary text--spaced" href="https://medium.com/@johnathan.l.simpson/maki-rarezhut-ed8eb173375d" target="_blank">Rea<l>d</l></a>
	</section>
</article>